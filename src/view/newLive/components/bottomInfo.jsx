...
const BottomInfo = (props) => {
  const { fetchUtils, dispatch, state: { live } } = useContextReducer.useContextReducer()
  const [gameList, gameListSet] = useState([]);
  const [giftList, giftListSet] = useState(live.giftData);
  ...
  const GiftsToWXKLW = (e) => {
    console.log('GiftsToWXKLW', e);
    visibleSet(true);
  };
  ...
  const sendGift = (num = 0, data) => {
    ...
    // giftLoadingSet(true);
    // bagProp背包道具  1 是背包道具 , 0 不是
    SendGift({ combo: num, count: num, liveId, gid, goldCoin, anchorId, bagProp: 0, }).then((rt) => {
      if (rt) {
  ...
  // 最新获取礼物数据
  const getGiftType = () => {
    giftGroupF(live.giftData, typeIndex)
    typeGiftListSet(live.giftData)
    giftListSet(live.giftData)
  };
  ...
  const getLiveGift = () => {
    ...
    if (!giftList.length) {
      ...
      giftListSet(giftList.sort((a, b) => a.goldCoin - b.goldCoin));
  ...
  const getHotGift = () => {
    ...
    return <div className="gift-group">
      {hotGift.length > 0 && hotGift.slice(0, 8).map((item, index) => {
        return <Popover overlayClassName="gift-popover" color="rgba(23, 23, 35)" onOpenChange={() => numSet(8)} placement="topRight" content={getGiftInputGroup(item)} key={`${item.cover}_${item.goldCoin}_${index}`}>
          <div className="gift-item" onClick={() => { sendGift(1, item) }}>
            <img className="icon " src={item.cover} alt="" />
      ...
      <div className="all-game-list">
        ...
        >
          {typeGiftList.map((item, index) => {
            ...
            }} className={`title ${typeIndex == index && 'active'}`} title={i18n.language === "vie" ? item.ynLan : item.chLan} key={`${item.ynLan}_${index}`}>
        ...
        {giftGroup.length ? (
          ...
          giftGroup.map((val, key) => {
            return (
              ...
              <div className="giftGroup" key={`group${key}`}>
                {val.map((item, index) => {
                  return (
                    <div key={`${item.gid}_${index}`} className={active.key === key && active.index === index ? "actives2" : "giftBox"} onClick={() => { }}>
                      <Popover overlayClassName="gift-popover" color="rgba(23, 23, 35)" onOpenChange={() => numSet(8)} placement="topRight" content={getGiftInputGroup(item)} >
                        <div className="all-game-list-item gift-item" onClick={() => sendGift(1, item)}>
                          <img className="game-icon " src={item.cover} alt="" />
