...
export default (props) => {
    ...
    const [gameList, setGameList] = useState(RoomGameList);
    ...
    const [configBase, configBaseSet] = useState(baseInfo);
    ...
    const getGameList = async () => {
        try {
            ...
            if (!(rt instanceof Error)) {
                ...
                let fbGame = rt[1]
                setLoading(false)
                const tempGameList = gameList.filter((item) => {
                    //过滤五分快三 越南30秒
                    return item.name !== 'jsks5' && item.name !== 'yncp30s'
                });
                // if (fbGame) {
                //     tempGameList.push({
                //         id: 0,
    ...
    useEffect(() => {
        configBaseSet(baseInfo)
    }, [baseInfo])
    const init = useCallback(() => {
        if (!RoomGameList.length) {
            getGameList();
        }
        // getGiftType();
        // getLiveGift();
    ...
    return <div className={`container-game-list ${loading && 'loading'}`}>
        ...
        <div className='game-detail-box'>
            <CSSTransition in={gameShow} timeout={500} classNames="fade" unmountOnExit appear={true}>
                ...
                <GameIframe currentGame={gameName} onClose={() => gameShowSet(false)}/>
            </CSSTransition>
        </div>
    </div>
}