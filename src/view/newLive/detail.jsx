...
import { GetUserCard, GetLiveRecharge, GetCurFlagOpenGame, GetLiveList, liveListV2, LiveHeart } from "../../api/live";
...
class LiveDetail extends Component {
  ...
  async checkPking(query) {
    ...
    if (!(res instanceof Error)) {
      let [data] = res?.listDataVos.filter((item) => item.liveListAnchorInfoVO.anchorId == query.anchorId);
      if (data?.liveListAnchorInfoVO?.pking && query?.pking != "true") {
        this.checkPcStatus(1);
      }
    }
  ...
  setCrowdFunding({ gid, avatar }) {
    let { crowdfundingList, avatarList } = this.state.crowdfundingData
  ...
  onShowResult(gameResult) {
    ...
    if (protocol == 29) {
      ...
      this.setCrowdFunding(gameResult)

      this.addAniList(gameResult);

      this.svgaPlay(gameResult)
      ...
      if (showType == 0) {
        // alert('111')
        this.setState({
          userLevels: gameResult.userLevel,
          gameResults: gameResult
        })
      }
    }
    else if (protocol == 21) {
      console.log('房间变更---------', gameResult);
      // alert('111')
    }
  }
  ...
  addAniList(gift) {
    ...
    console.log('showAniList--------------', showAniList);
    this.playerDom.setState({ showAniList });
  }

  svgaPlay(gids) {
    console.log('svgaPlay---giftData', gids);
    this.setState({
      gid: gids.gid
    })
  }
  ...
  render() {
    let { baseInfo, gameResult, pikingResult, giftList, userInfo, FreshUser, modalVisible, modalContent, waterTop,
      fbShow, fbIsShow, gameResultList, isAd, urlSearchD, crowdfundingVisible, crowdfundingData } = this.state;
    let { t } = this.props;
    ...
    return (
      <div className={"live-detail-container " + (baseInfo.isAd == 1 && "w-1200")}>
        <Redux getFreshUser={this.getFreshUser} getUser={this.getUser} getLive={this.getLive} />
        ...
        {/* 蒙尘 */}
        {this.state.needPay && <div className='disgraceful'>
        </div>}


        {/* {Local("userInfo2")?.manage != 1 && <EnterPay displaysSet={(e) => this.setState({ displays: e })} needPaySet={(e) => {
          this.setState({
            needPay: e,
          })
        }} />} */}
              ...
              <div className="live-detail-left-con-r">
                ...
                <div className="live-detail-left-con-r-t" style={{ zIndex: 3, position: "relative" }}>
                  <div className="live-detail-left-con-r-t-player" style={{ position: "relative" }}>
                    {
                      // this.state.suspendDataSet ? suspendDataF() :
                      baseInfo.anchorId && <Player waterTop={waterTop} giftList={giftList} pikingResult={pikingResult} getPlayerDom={this.getPlayerDom} gameResult={gameResult} gameResultList={gameResultList} baseInfo={baseInfo} />}
                    {/* <GiftsToWX giftsToWX={this.state.giftsToWX} lang={i18n.language} /> */}
                  </div>
                  {
                    crowdfundingVisible ? <Crowdfunding baseInfo={baseInfo}
                      onHiddenButton={() => this.setState({ waterTop: '24px' })}
                      onClose={() => this.setState({ crowdfundingVisible: false, waterTop: '24px' })} info={crowdfundingData} /> : ''
                  }

                  {/* 礼物svga */}
                  <SVGAPlay gid={this.state.gid} unmixed={() => this.setState({ gid: '' })} />
                  {/* 进场飘屏 */}
                  <ShowTypes
                    userLevels={this.state.userLevels}
                    gameResults={this.state.gameResults}
                    unmixed={() => this.setState({ gameResults: {}, userLevels: '' })}
                  />
                  {/* 密码房 */}
                  {/* <EnterPass /> */}
                  {/* 付费房 */}
                  {/* {Local("userInfo2")?.manage != 1 && <EnterPay displaysSet={(e) => this.setState({ displays: e })} needPaySet={(e) => this.setState({ needPay: e })} />} */}
                </div>
                {/* 礼物 */}
