...
export default () => {
    ...
    const getContent1 = () => {
        return <div>
            ...
            >
                ...
                {
                    list1.map((item, index) => (<div onClick={() => choice1(item)} className='betting-item' key={index}>
    ...
    const getIMG = (name, list) => {
        ...
        return list.map((item, index) => (
            map ? <img key={index} src={map[item]} /> : <div key={index} className={'iconNum ' + (name == 'yflhc' && 'lhc')}>{item}</div>
        ))
    }
