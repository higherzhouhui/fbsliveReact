...
const GiftsToWX = (props) => {
    ...
    return <div>
        {/* 入口 */}
        {contactFlag == 1 && <div className={style.GiftsToWX} onClick={() => onLuckBag()}>
            ...
            visible && <div onClick={() => { visibleSet(false) }} >
                <div className={style.GiftsToWX_center} onClick={(e) => { e.stopPropagation() }}>
                    ...
                    <div className={style.GiftsToWX_information}>
                        ...
                        {getAnchorCardD?.isComplete == false && <Button className={style.GiftsToWX_information_but} onClick={() => { visibleSet(false), window.eventBus.emit('GiftsToWXKLW') }}>{t('huoqu')}</Button>}
